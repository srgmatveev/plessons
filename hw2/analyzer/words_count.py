...
def get_top_functions_in_path(path, top_size=10):
    filenames = get_file_names(path)
    trees = get_syntax_trees(filenames)
    func_names = get_all_functions_names(trees)
...
def return_nodes(tree, ast_type):
    nodes = []
    if ast_type == ast.Name:
        iter_children = ast.walk(tree)
    else:
        iter_children = ast.iter_child_nodes(tree)
...
def get_nodes_loc_vars(tree):
    funcs = return_nodes(tree=tree, ast_type=ast.FunctionDef)
    func_vars = make_flat_list(
        [return_nodes(tree=node, ast_type=ast.Name) for node in funcs])
    #print(func_vars)
    for var in func_vars:
        print(var.id)


def get_nodes_ast_all_loc_vars(trees):
    functions = []
    for t in trees:
        get_nodes_loc_vars(t)
    return make_flat_list(functions)


def get_top_local_vars(path, top_size=10):
    filenames = get_file_names(path)
    trees = get_syntax_trees(filenames)
    loc_names = get_nodes_ast_all_loc_vars(trees)
